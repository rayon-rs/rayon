...
pub fn parse_tsp_data(text: &str) -> Result<Graph, String> {
    ...
    while let Some(header) = parse_tsp_header(&mut data) {
        match header.name {
            ...
            "TYPE" => {
                if header.value != "TSP" {
                    return Err(format!("line {}: expected \"TSP\" for TYPE, not {:?}",
                                       header.line,
                                       header.value));
                }
            }
            "DIMENSION" => {
                match usize::from_str(header.value) {
                    ...
                    Err(_) => {
                        return Err(format!("line {}: expected an integer for DIMENSION, not {:?}",
                                           header.line,
                                           header.value));
                    }
                }
            ...
            "EDGE_WEIGHT_TYPE" => {
                if header.value != "EUC_2D" {
                    return Err(format!("line {}: expected \"EUC_2D\" for EDGE_WEIGHT_TYPE, not \
                                        {:?}",
                                       header.line,
                                       header.value));
                }
    ...
    for i in graph.all_nodes() {
        let coord_i = match coords.get(&i) {
            Some(v) => v,
            None => {
                return Err(format!("line {}: never found coordinate for node {}",
                                   data.line_num,
                                   i.index()))
            }
        };
