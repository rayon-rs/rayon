...
fn fact(n: u32, threshold: usize) -> BigUint {
    (1 .. n + 1)
        .into_par_iter()
        .map(BigUint::from)
...
fn factorial_par_iter(b: &mut test::Bencher) {
    ...
    let f = factorial(N);
    b.iter(|| assert_eq!(fact(test::black_box(N), 1), f));
}


#[bench]
/// Compute the Factorial using rayon::par_iter with seq threshold of 200.
fn factorial_par_iter_threshold_200(b: &mut test::Bencher) {
    rayon::initialize(Configuration::new())
                     .expect(INIT_FAILED);

    let f = factorial(N);
    b.iter(|| assert_eq!(fact(test::black_box(N), 200), f));
}
