...
impl<'c, ITEM: Send> CollectConsumer<'c, ITEM> {
    ...
    pub unsafe fn new(writes: &'c AtomicUsize,
        ...
        CollectConsumer { writes: writes, target: target, offset: 0, len: len }
    }
}
...
impl<'c, ITEM: Send> Consumer for CollectConsumer<'c, ITEM> {
    ...
    fn split_at(self, index: usize) -> (Self, Self) {
        ...
        let CollectConsumer { writes, target, offset, len } = self;
    ...
    fn consume(&mut self, _: (), item: ITEM) {
    ...
    fn complete(self, _: ()) {
        let CollectConsumer { writes, target: _, offset, len } = self;
